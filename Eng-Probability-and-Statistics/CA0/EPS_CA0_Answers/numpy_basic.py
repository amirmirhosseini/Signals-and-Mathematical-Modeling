...
def multiples_of_ten(start, stop):
  ...
  x = np.arange(start, stop+1, 10, dtype=np.float64)
...
def reshape_practice(x):
  ...
  #############################################################################
  # Replace "pass" statement with your code
  y= np.reshape(x,(2,3,4)).transpose(1,0,2).reshape((3,8))
  #############################################################################
  #                            END OF YOUR CODE                               #
  #############################################################################
  return y
...
def normalize_columns(x):
  ...
  # Replace "pass" statement with your code
  s = x.shape[0]
  colMeans = np.sum(x, axis=0) / s
    
  var = np.sum((x - colMeans)**2, axis=0) / s
  stdDev= np.sqrt(var)
